...
import { STATUS_LABEL, statusBadge as badge, type VerificationRequest } from '../lib/requestModel'
import { prepareResultEmail, prepareHoldingDraft } from '../lib/emailService'
import { getService, getRequestField, resultLabel, resultBadge, type FieldKey } from '../lib/services'
...
export default function AdminRequestDetail() {
  ...
  const [emailEdited, setEmailEdited] = useState(false) // admin changed the message text by hand
  ...
  useEffect(() => {
    ...
    if (!v) {
      const holding = prepareHoldingDraft(request)
      setEmailSubject(holding.subject)
      setEmailBody(holding.text)
      setEmailHtml('')
      setTemplateId('')
      return
    }
    const prepared = prepareResultEmail(request, v)
  ...
  const validateBeforeSave = (): Record<string, string> => {
    ...
    } else if (outcome && service.results.find(r => r.value === outcome)?.requiresNotes) {
      if (!otherNotes.trim()) {
    ...
    } else if (outcome && otherNotes.length > 2000) {
      e.otherNotes = 'Notes are too long (max 2000 characters).'
    }
    return e
  }
  ...
  const handleSaveResult = async () => {
    ...
    try {
      const payload: any = {
        outcome,
        notes: (outcome === 'CHALLAN_FOUND' ? f.notes.trim() : otherNotes.trim()) || undefined,
      }
      if (outcome === 'CHALLAN_FOUND') {
      ...
      const json = await res.json().catch(() => null)
      if (!res.ok || !json?.success) throw new Error(json?.error?.message || `Failed (${res.status})`)
      setSaveOk(`Result saved as ${resultLabel(outcome)} — request is now Result Ready. Preview and send the email below.`)
      setShowPreview(true)
  ...
  const handleSendEmail = async () => {
    ...
    if (!verification) { setSendError('Save a verification result first.'); return }
    if (!inSync) { setSendError('The result has unsaved changes. Save the result first so the email matches it.'); return }
    ...
    try {
      const res = await fetch(`/api/admin/requests/${encodeURIComponent(id)}/email`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ subject: emailSubject.trim(), text: emailBody, html: emailEdited ? '' : emailHtml, templateId: templateId || verification.outcome })
      })
      const json = await res.json().catch(() => null)
  ...
  const selectedOption = service.results.find(r => r.value === outcome)
  // The email may only be sent for a result that is saved AND matches what is currently on screen.
  const CHALLAN_DETAIL_KEYS = ['challanNumber', 'violation', 'challanDate', 'challanTime', 'location', 'fineAmount', 'dueDate', 'paymentStatus', 'referencePsid'] as const
    ...
    <div className="space-y-6 overflow-x-hidden">
      ...
      <div className="bg-white rounded-[20px] border border-[#0C1E3A]/5 shadow-sm overflow-hidden">
        <div className="px-5 sm:px-6 py-5 flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`rounded-full border px-3 py-1 text-[11px] font-[800] tracking-wide ${service.tag}`}>{service.badge}</span>
              ...
              {hasResult && <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-[800] ${resultBadge(verification!.outcome)}`}><BadgeCheck size={12} /> Result: {resultLabel(verification!.outcome)}</span>}
            </div>
        ...
        <div className="px-5 sm:px-6 py-3 bg-[#F8FAFC] border-y border-[#0C1E3A]/5 flex flex-wrap items-center gap-3 text-[12px]">
          ...
          <span className="mono text-[11px] bg-white border border-[#0C1E3A]/10 rounded-full px-2.5 py-1">NEW → IN REVIEW → RESULT READY → COMPLETED</span>
          <span className="text-[#5B6B85] hidden sm:inline">Verification workflow • Resend available after COMPLETED</span>
      ...
      <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-6 items-start">
        <div className="space-y-6 min-w-0">
          <div className="bg-white rounded-[18px] border border-[#0C1E3A]/5 shadow-sm overflow-hidden">
            ...
            <div className="p-5 sm:p-6 grid sm:grid-cols-2 gap-4 text-[13px]">
              {service.detail.map(key => {
                const raw = getRequestField(request, key)
                const label = TILE_LABEL[key]
                if (key === 'cnic') {
                  const d = raw.replace(/\D/g, '')
                  return (
                    <div key={key} className="rounded-xl bg-amber-50/60 border border-amber-200 p-4"><div className="text-[11px] font-[800] tracking-[0.08em] text-[#92400E] flex items-center gap-1"><ShieldCheck size={12} /> {label}</div><div className="mono font-[700] mt-1 break-all">{raw ? (reveal ? d.replace(/(\d{5})(\d{7})(\d{1})/, '$1-$2-$3') : maskCNIC(d)) : '—'}</div><div className="text-[11px] text-[#92400E]/70 mt-1">HttpOnly session • Masked by default</div></div>
                  )
                }
                if (key === 'phone') {
                  const d = raw.replace(/\D/g, '')
                  return (
                    <div key={key} className="rounded-xl bg-amber-50/60 border border-amber-200 p-4"><div className="text-[11px] font-[800] tracking-[0.08em] text-[#92400E] flex items-center gap-1"><Phone size={12} /> {label}</div><div className="mono font-[700] mt-1">{raw ? (reveal ? d.replace(/(\d{4})(\d{7})/, '$1-$2') : maskMobile(d)) : '—'}</div><div className="text-[11px] text-[#92400E]/70 mt-1">Never in URL • Server-protected</div></div>
                  )
                }
                if (key === 'email') {
                  return (
                ...
                const mono = key === 'challanNumber' || key === 'complaintNumber'
                return (
                  <div key={key} className="rounded-xl bg-[#F8FAFC] border border-[#0C1E3A]/5 p-4"><div className="text-[11px] font-[800] tracking-[0.08em] text-[#5B6B85]">{label}</div><div className={`font-[700] mt-1 break-words ${mono ? 'mono' : ''}`}>{raw || '—'}</div></div>
                )
              })}
            </div>
          </div>

          {isLegacyVehicleRequest && (
          <div className="bg-white rounded-[18px] border border-[#0C1E3A]/5 shadow-sm overflow-hidden">
          ...
          <div className="rounded-[18px] bg-[#0C1E3A] text-white p-5 sm:p-6 relative overflow-hidden">
            ...
            <div className="relative flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[13px] font-[800]">Verification Required</div>
                <p className="text-[13px] leading-6 text-white/80 mt-1 text-pretty">Check the submitted details against the relevant official source, then record the result below.</p>
        ...
        <div className="space-y-6 min-w-0">
          <div className="bg-white rounded-[18px] border border-[#0C1E3A]/5 shadow-sm overflow-hidden">
            <div className="px-5 sm:px-6 h-11 flex items-center justify-between bg-[#F8FAFC] border-b border-[#0C1E3A]/5">
              ...
              <span className={`text-[11px] font-[800] rounded-full border px-2.5 py-1 shrink-0 ${hasResult ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>{hasResult ? `Saved: ${resultLabel(verification!.outcome)}` : 'Awaiting result'}</span>
            </div>
            <div className="p-5 sm:p-6 space-y-4">
              <div>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {service.results.map((r, i) => (
                    <button key={r.value} type="button" onClick={() => { setOutcome(r.value); setFieldErrors({}); setSaveError(null) }} className={`h-10 rounded-xl border text-[12px] font-[700] transition active:scale-[0.98] ${service.results.length % 2 === 1 && i === service.results.length - 1 ? 'col-span-2' : ''} ${outcome === r.value ? 'bg-[#0C1E3A] text-white border-[#0C1E3A] shadow' : 'bg-white border-[#0C1E3A]/10 hover:bg-[#F8FAFC] hover:border-[#0C1E3A]/15'}`}>{r.short || r.label}</button>
                  ))}
                </div>
                ...
                <p className="mt-2 text-[11px] text-[#5B6B85] flex gap-1"><Info size={12} className="mt-0.5 shrink-0" /> Determines the email template. Choose based on the verification outcome.</p>
              </div>
              ...
              {outcome && outcome !== 'CHALLAN_FOUND' && (
                <div className="pt-3 border-t border-[#0C1E3A]/5 space-y-2">
                  <label className="text-[11px] font-[700]">{outcome === 'MORE_INFORMATION_REQUIRED' ? 'What information is required? *' : outcome === 'UNABLE_TO_VERIFY' ? 'Reason — why unable to verify? *' : selectedOption?.requiresNotes ? 'Notes / explanation *' : 'Notes for the user (optional)'}</label>
                  <textarea value={otherNotes} onChange={e => { setOtherNotes(e.target.value); if (fieldErrors.otherNotes) setFieldErrors(prev => ({ ...prev, otherNotes: '' })) }} rows={4} placeholder={outcome === 'MORE_INFORMATION_REQUIRED' ? 'e.g., Please reply with the challan slip photo and confirm your CNIC…' : outcome === 'UNABLE_TO_VERIFY' ? 'e.g., Portal unavailable / details did not match — try again later…' : outcome === 'NO_CHALLAN_FOUND' ? 'e.g., No active challan as of verification time…' : 'e.g., Any detail the user should know about this result…'} className={`w-full rounded-xl border px-3 py-2 text-[13px] leading-6 outline-none resize-none focus:ring-2 ${fieldErrors.otherNotes ? 'border-red-300 focus:border-red-400 focus:ring-red-100' : 'border-[#0C1E3A]/15 focus:border-[#0C1E3A] focus:ring-[#0C1E3A]/10'}`} />
                  {fieldErrors.otherNotes ? <p className="text-[11px] text-red-600 flex items-center gap-1"><AlertCircle size={11} /> {fieldErrors.otherNotes}</p> : <p className="text-[11px] text-[#5B6B85]">{selectedOption?.requiresNotes ? 'Sent verbatim to user — keep clear. Private notes below are never sent.' : `${service.resultNotesHelp} Private notes below are never sent.`}</p>}
                </div>
              )}
          ...
          <div className="bg-white rounded-[18px] border border-[#0C1E3A]/5 shadow-sm overflow-hidden">
            ...
            <div className="p-5 sm:p-6 space-y-4">
              <div className="rounded-xl bg-[#F8FAFC] border border-[#0C1E3A]/5 p-3 text-[12px] space-y-2">
                ...
                {hasResult && <div className="flex items-center justify-between gap-2"><span className="font-[700]">Result</span><span className={`rounded-full border px-2.5 py-1 text-[11px] font-[800] ${resultBadge(verification!.outcome)}`}>{resultLabel(verification!.outcome)}</span></div>}
              </div>
              ...
              <div>
                ...
                <textarea value={emailBody} onChange={e => { setEmailBody(e.target.value); setEmailEdited(true) }} rows={10} className="mt-1 w-full rounded-xl border border-[#0C1E3A]/15 px-3 py-2 text-[13px] leading-6 outline-none resize-none focus:border-[#0C1E3A] focus:ring-2 focus:ring-[#0C1E3A]/10" />
              </div>
              ...
              <div className="flex gap-2">
                <button disabled={!hasResult} onClick={() => setShowPreview(!showPreview)} title={!hasResult ? 'Save result first' : ''} className="flex-1 h-10 rounded-full border border-[#0C1E3A]/10 bg-white text-[13px] font-[700] inline-flex items-center justify-center gap-1.5 hover:bg-[#F8FAFC] active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed">
                  {showPreview ? <><EyeOff size={14} /> Hide Preview</> : <><Eye size={14} /> Preview Email</>}
                </button>
                <button disabled={sending || !hasResult || !inSync || !emailSubject.trim() || !emailBody.trim()} onClick={handleSendEmail} title={!hasResult ? 'Save result first' : !inSync ? 'Save the changed result first' : ''} className="flex-1 h-10 rounded-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-[#94A3B8] disabled:opacity-60 text-white text-[13px] font-[800] inline-flex items-center justify-center gap-1.5 shadow active:scale-[0.98]">
              ...
              {hasResult && !inSync && <p className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5">The result on screen has changes that are not saved. Save the result again to update the email before sending.</p>}

              {showPreview && (
                <div className="rounded-xl border border-[#0C1E3A]/10 bg-[#FDFDFB] p-4 space-y-3">
                  <div className="flex items-center justify-between"><div className="text-[11px] font-[800] tracking-[0.08em] text-[#5B6B85]">EMAIL PREVIEW</div><span className={`text-[11px] font-[700] rounded-full border px-2.5 py-1 ${hasResult ? resultBadge(verification!.outcome) : 'bg-slate-50'}`}>{hasResult ? resultLabel(verification!.outcome) : 'No result'}</span></div>
              ...
              {sendSuccess && (
                <div className="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 flex gap-2.5 text-[13px] leading-5 text-emerald-900">
                  ...
                  <div>
                    ...
                    <div className="text-[12px] break-words">To {request.email} • {resultLabel(verification!.outcome)} • Status: SENT {sendSuccess.messageId && `• ${sendSuccess.messageId}`}</div>
                  </div>
                </div>
              )}
              {sendError && (
                  ...
                  {emails.length === 0 ? <div className="text-[12px] text-[#5B6B85] text-center py-6 border border-dashed rounded-xl">No emails yet. After saving a result, preview and send — history appears here.</div> : emails.map(e => (
                    <div key={e.id} className={`rounded-xl border p-3 ${e.status === 'SENT' ? 'bg-white border-emerald-200' : 'bg-red-50 border-red-200'}`}>
                      ...
                      <div className="text-[11px] text-[#5B6B85] mt-1 space-y-0.5 break-words">
                        <div>To <span className="mono font-[600]">{e.to}</span> • {resultLabel(e.resultType || e.templateId) || '—'}</div>
