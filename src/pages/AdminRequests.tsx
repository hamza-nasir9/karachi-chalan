...
import { RequestStatus, STATUS_LABEL, type RequestStatusType, type VerificationRequest } from '../lib/requestModel'
...
const FILTERS: Array<{ label: string; value: RequestStatusType | 'ALL' }> = [
  ...
  { label: 'Challan Found', value: RequestStatus.CHALLAN_FOUND },
  { label: 'No Challan Found', value: RequestStatus.NO_CHALLAN_FOUND },
  { label: 'Unable to Verify', value: RequestStatus.UNABLE_TO_VERIFY },
  { label: 'More Info Required', value: RequestStatus.MORE_INFO_REQUIRED },
      ...
      <div className="bg-white rounded-[18px] border border-[#0C1E3A]/5 p-4 flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between shadow-sm">
        <div className="relative flex-1 max-w-[560px]">
          ...
          <input
            ...
            placeholder="Search: Request ID, Name, Vehicle, Email, Mobile, CNIC"
              ...
              {loading ? (
                ...
                <tr key={r.requestId} className="hover:bg-[#F8FAFC] transition-colors">
                  ...
                  <td className="px-4 py-4">
                    ...
                    <div className="text-[11px] text-[#5B6B85] mt-1">{r.vehicleType}{r.vehicleColor ? ` • ${r.vehicleColor}` : ''}</div>
                  </td>
                  <td className="px-4 py-4">
